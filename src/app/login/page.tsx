...
export default function LoginPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('waptrix_last_login') as 'google' | 'email' | null;
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError(null);
    try {
      localStorage.setItem('waptrix_last_login', 'google');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('waptrix_last_login', 'email');
      ...
      <div className="flex-1 flex flex-col justify-center px-8 sm:px-12 lg:px-24 xl:px-32 z-10">
        <div className="max-w-md w-full mx-auto space-y-10">
          ...
          <div className="relative">
            {lastUsed === 'google' && (
              <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-jade text-background text-[10px] font-bold px-2.5 py-0.5 rounded-full z-10 whitespace-nowrap">
                Last used
              </span>
            )}
            <button
              ...
              className={`w-full flex items-center justify-center gap-3 py-4 px-6 bg-surface border rounded-2xl text-sm font-semibold text-text-primary hover:bg-card transition-all disabled:opacity-50 disabled:cursor-not-allowed ${lastUsed === 'google' ? 'border-jade/50 hover:border-jade/70' : 'border-border hover:border-jade/30'}`}
            >
              {googleLoading ? (
