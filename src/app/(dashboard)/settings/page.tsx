...
export default function SettingsPage() {
  ...
  const [isOwner, setIsOwner] = useState(true); // staff can't manage team
  ...
  useEffect(() => {
    fetchWaProfile();
    // Load team members (silently — owners only)
    axios.get('/api/team')
      .then(r => { setTeamMembers(Array.isArray(r.data) ? r.data : []); })
      .catch(err => {
        if (err?.response?.status === 403) setIsOwner(false); // staff account
      });
    // Load automations
    axios.get('/api/automations')
      .then(r => { if (Array.isArray(r.data)) setAutomations(r.data); })
      .catch(() => {});
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20">
      ...
      <section className="pt-8 border-t border-border">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-8 h-8 bg-jade/10 border border-jade/20 rounded-lg flex items-center justify-center">
            <Users className="w-4 h-4 text-jade" />
          </div>
          <div>
            <h3 className="text-lg font-bold font-syne">Team Members</h3>
            <p className="text-xs text-text-muted">Invite staff to manage the shared inbox and campaigns.</p>
          </div>
        </div>
        ...
        {!isOwner ? (
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card">
              <p className="text-sm font-bold mb-4 font-syne">Invite a team member</p>
              <div className="flex gap-3 flex-wrap">
                <input
                  ...
                  className="flex-1 min-w-[200px] bg-surface border border-border rounded-xl px-4 py-2.5 text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-jade/50"
                />
                <select
                  value={inviteRole}
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setTeamMembers(prev => {
                        ...
                        return exists ? prev.map((m: any) => m.email === r.data.email ? r.data : m) : [r.data, ...prev];
                      });
                      setInviteEmail("");
                      setInviteMsg({ type: "success", text: `Invite sent to ${r.data.email}` });
                    } catch (err: any) {
              ...
              {inviteMsg && (
                <p className={`mt-3 text-xs font-semibold ${inviteMsg.type === "success" ? "text-jade" : "text-danger"}`}>
                  {inviteMsg.text}
                </p>
              )}
            </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-surface border-b border-border">
                      <th className="px-5 py-3 text-[10px] font-bold text-text-muted uppercase tracking-widest">Email</th>
                      ...
                      <th className="px-5 py-3 text-[10px] font-bold text-text-muted uppercase tracking-widest text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/50">
                    {teamMembers.map((m: any) => (
                      <tr key={m.id} className="hover:bg-card/40 transition-colors">
                        <td className="px-5 py-3 text-sm text-text-primary">{m.email}</td>
                        <td className="px-5 py-3">
                          <select
                            value={m.role}
                            onChange={async (e) => {
                              await axios.patch(`/api/team/${m.id}`, { role: e.target.value });
                              setTeamMembers(prev => prev.map((x: any) => x.id === m.id ? { ...x, role: e.target.value } : x));
                            }}
                        ...
                        <td className="px-5 py-3">
                          <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold ${m.status === "active" ? "bg-jade/10 text-jade" : "bg-warning/10 text-warning"}`}>
                        ...
                        <td className="px-5 py-3 text-right">
                          <button
                            onClick={async () => {
                              ...
                              await axios.delete(`/api/team/${m.id}`);
                              setTeamMembers(prev => prev.filter((x: any) => x.id !== m.id));
                            }}
            ...
            {teamMembers.length === 0 && (
              <p className="text-sm text-text-muted">No team members yet. Invite someone above.</p>
            )}
          </div>
        )}
      </section>
      ...
      <section className="pt-8 border-t border-border">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-lg font-bold font-syne">Automations</h3>
            <p className="text-xs text-text-muted">Auto-reply to incoming WhatsApp messages.</p>
          </div>
        </div>
          ...
          {[
            {
              ...
              subtitle: "Sent automatically when someone messages you for the first time.",
            ...
            },
            {
              ...
              subtitle: "Sent when a message arrives outside your business hours.",
              placeholder: "We're currently outside office hours. We'll respond first thing when we're back! 🙏",
              hasSchedule: true,
            },
          ].map(({ type, icon: Icon, title, subtitle, placeholder, hasSchedule }) => {
            const auto = automations.find((a: any) => a.type === type) ?? {
              type, enabled: false, message: "", ooo_start: 18, ooo_end: 9, timezone: "Asia/Kolkata",
            };

            const save = async (patch: object) => {
              const updated = { ...auto, ...patch };
              setAutomations(prev => prev.map((a: any) => a.type === type ? updated : a).concat(
                prev.find((a: any) => a.type === type) ? [] : [updated]
              ));
              setAutoSaving(type);
              try {
                const r = await axios.post('/api/automations', updated);
                setAutomations(prev => prev.map((a: any) => a.type === type ? r.data : a));
              } catch { /* silent */ } finally { setAutoSaving(null); }
            };

            return (
              <div key={type} className="glass-card space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-info/10 border border-info/20 rounded-xl flex items-center justify-center flex-shrink-0">
                  ...
                  <button
                    ...
                    className={`relative flex-shrink-0 w-11 h-6 rounded-full transition-colors ${auto.enabled ? "bg-jade" : "bg-surface border border-border"}`}
                  >
                    <span className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all ${auto.enabled ? "left-6" : "left-1"}`} />
                  </button>
                </div>
                  ...
                  <>
                    ...
                    {hasSchedule && (
                      <div className="flex flex-wrap gap-3 items-center">
                        <div className="flex items-center gap-2">
                          <label className="text-xs text-text-muted font-semibold">OOO from</label>
                          <select
                            value={auto.ooo_start ?? 18}
                            onChange={e => save({ ooo_start: parseInt(e.target.value) })}
                            className="bg-surface border border-border rounded-lg px-2 py-1.5 text-xs text-text-primary focus:outline-none"
                          >
                        ...
                        <div className="flex items-center gap-2">
                          <label className="text-xs text-text-muted font-semibold">to</label>
                          <select
                            value={auto.ooo_end ?? 9}
                            onChange={e => save({ ooo_end: parseInt(e.target.value) })}
                            className="bg-surface border border-border rounded-lg px-2 py-1.5 text-xs text-text-primary focus:outline-none"
                          >
                            {Array.from({ length: 24 }, (_, i) => (
                              <option key={i} value={i}>{i.toString().padStart(2, "0")}:00</option>
                        ...
                        <div className="flex items-center gap-2">
                          <label className="text-xs text-text-muted font-semibold">Timezone</label>
                          <select
