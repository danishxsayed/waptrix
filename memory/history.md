...
- **Opt-in Status Defaulting & Campaign Audience Expansion**:
  ...
  - Adjusted the batch dispatcher in [route.ts](file:///Users/danishsayed/Desktop/Waptrix/src/app/api/campaigns/[id]/process-batch/route.ts) to skip contacts only if they are explicitly opted out (`opted_in === false`) or confirmed as not on WhatsApp (`custom4 = 'not_on_whatsapp'`).
